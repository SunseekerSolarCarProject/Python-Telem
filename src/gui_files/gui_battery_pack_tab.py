...
class BatteryPackGraphTab(QWidget):
    """
    Battery‐pack telemetry, with:
      • Shift+double‐click near left axis → per‐key unit picker
      • Plain double‐click → pan/zoom toggle
      • Mouse wheel only zooms when in zoom‐mode; otherwise scrolls parent
    """
    def __init__(self, pack_name, keys, units, color_mapping):
        ...
        # per‐key overrides & last raw telemetry
        self.unit_overrides = {}
        self._last_raw      = {}

        # global metric/imperial maps
    ...
    def _init_ui(self):
        ...
        vbox.addWidget(QLabel(f"<b>{self.pack_name} Telemetry</b>"))
        self.graph_widgets = {}
    ...
    def _add_graph(self, layout, key, color):
        ...
        # all double‐clicks route through here
    ...
    def _display_unit(self, key):
        return (
            self.unit_overrides.get(key)
            or self.units_map.get(key)
            or KEY_UNITS.get(key, "")
        )
    ...
    def update_graphs(self, telemetry_data):
        ...
        for key, pw in self.graph_widgets.items():
            ...
            self.logger.debug(f"[{self.pack_name}] {key}: {raw} → {disp} {tgt}")

    def eventFilter(self, source, event):
        if event.type() == QEvent.Type.MouseButtonPress:
            child = source.childAt(event.position().toPoint())
            if not any(pw == child or pw.isAncestorOf(child)
