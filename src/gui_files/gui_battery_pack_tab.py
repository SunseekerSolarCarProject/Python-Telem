...
class BatteryPackGraphTab(QWidget):
    """
    A specialized tab for battery pack telemetry,
    similar structure to MotorControllerGraphTab and GraphTab.
    """
    ...
    def set_curve_color(self, key, color):
        if key in self.graph_widgets:
            ...
            self.logger.warning(f"Key '{key}' not found in '{self.pack_name}'")
    ...
    def update_graphs(self, telemetry_data):
        try:
            ...
            self.logger.error(f"Error updating graphs in '{self.pack_name}'. Exception: {e}")
