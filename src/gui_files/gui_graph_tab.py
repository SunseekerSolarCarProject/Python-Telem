...
class GraphTab(QWidget):
    """
    A tab for displaying generic telemetry as separate PyQtGraph plots with fixed heights and scrolling.
    """
    def __init__(self, tab_name, keys, units, logger, color_mapping):
        super().__init__()
    ...
    def set_curve_color(self, key, color):
        ...
        if key in self.graph_widgets:
            ...
            self.logger.warning(f"Attempted to set color for unknown key '{key}' in {self.tab_name}")
