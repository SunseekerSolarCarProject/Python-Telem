...
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QLabel, QScrollArea, QInputDialog, QApplication
...
class MotorControllerGraphTab(QWidget):
    ...
    def _init_ui(self):
        ...
        vbox.addWidget(QLabel(f"<b>{self.controller_name} Telemetry</b>"))
        self.graph_widgets = {}

        for key in self.keys:
            pen = self.color_mapping.get(key, 'gray')
            self._add_graph(vbox, key, pen)
    ...
    def _on_double_click_unit(self, key):
        ...
        unit, ok = QInputDialog.getItem(
            self, f"Select unit for {key}", "Unit:", choices, idx, False
        )
        if not ok:
            return
    ...
    def update_graphs(self, telemetry_data):
        ...
        for key, pw in self.graph_widgets.items():
            ...
            self.logger.debug(f"[{self.controller_name}] {key}: {raw} → {disp} {tgt}")
